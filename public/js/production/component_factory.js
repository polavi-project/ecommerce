...
function ComponentFactory(id, ReactComponent) {
    ...
    _this.addChild = function (ReactElement, sort_order) {
        if (React.isValidElement(ReactElement) === false) throw new Error("Invalid component");
        if (sort_order === undefined) sort_order = 0;
        let s = parseInt(sort_order);
        if (s < 0) return;

        if (_this.children[s] === undefined) _this.children[s] = [];
    ...
    _this.create = function create() {
        ...
        Component$Wrapper.prototype.render = function render() {
            return React.createElement(ReactComponent, _extends({ child: child }, this.props));
        };
