...
const Fetch = (url, pushState = false, method = "GET", data = {}, onStart = null, onComplete = null, onError = null, onFinally = null) => {
    ...
    fetch(url, config).then(response => {
        ...
        // Alerts
        if (response.alerts) store.dispatch({ 'type': ADD_ALERT, 'payload': { alerts: response.alerts } });

        // App state
        if (response.appState) store.dispatch({ 'type': ADD_APP_STATE, 'payload': { appState: response.appState } });
        ...
        Promise.all(promises).then(() => {
            if (widgets.length > 0) store.dispatch({ 'type': UPDATE_WIDGETS, 'payload': { widgets: widgets, isNewPage: response.isNewPage === true } });
        }).catch(e => {
            console.log(e);
