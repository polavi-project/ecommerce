...
const Fetch = (url, pushState = false, method = "GET", data = {}, onStart = null, onComplete = null, onError = null, onFinally = null) => {
    ...
    fetch(url, config).then(response => {
        ...
        if (pushState === true) store.dispatch({ 'type': SET_ALERT, 'payload': { alerts: _.get(response, 'alerts', []) } });else store.dispatch({ 'type': ADD_ALERT, 'payload': { alerts: _.get(response, 'alerts', []) } });
