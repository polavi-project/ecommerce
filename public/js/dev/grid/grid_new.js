import Area from "../../production/area.js";
...
function IdColumn({areaProps}) {
    ...
    return <div className={"column"}>
        ...
        {areaProps.rows.map((r, i) => {
            return <div className={"row"} key={i}><span>{r.category_id}</span></div>;
        })}
    </div>
}
...
export default function Grid({id, apiUrl})
{
    ...
    const applyFilter = () => {
        ...
        axios({
            ...
            if(_.get(response, 'data.payload.data.categoryCollection.categories')) {
                setRows(_.get(response, 'data.payload.data.categoryCollection.categories'));
            }
    ...
    return <div id={id} className={"grid grid-container"}>
        <table>
            <tr>
                <Area
                    id={id + "-grid-header"}
