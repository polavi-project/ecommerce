...
function Form(props) {
    ...
    const [token1, setToken1] = React.useState(()=> {
        let token = PubSub.subscribe(FORM_FIELD_CREATED, function(message, data) {
            if(data.name === undefined || data.validation_rules === undefined || data.disabled === true || data.formId !== props.id)
                return;
    ...
    return <form ref={formRef} id={props.id} method="POST" onSubmit = {(e)=>onSubmit(e)} className={props.className}>
        {props.children}
        {props.submitText !== null &&
        <div className="polavi-form-button form-submit-button">
            <a href="javascript:void(0)" className="btn btn-primary" onClick={ () => { formRef.current.dispatchEvent(new Event('submit')) } }>
