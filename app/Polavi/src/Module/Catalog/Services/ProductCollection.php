...
class ProductCollection extends CollectionBuilder
{
    ...
    public function __construct(Container $container)
    {
        ...
        if (!$container->get(Request::class)->isAdmin()) {
            ...
            if ($setting == 0) {
                $collection->where('product.manage_stock', '=', 0, "(")
                    ->orWhere('product.qty', '>', 0, '(')
                    ->andWhere('product.stock_availability', '=', 1, null, '))');
            }
        }
        if (!$container->get(Request::class)->isAdmin()) {
            $customerGroupId = $container->get(Request::class)
                ->getCustomer()->isLoggedIn() ?
                $container->get(Request::class)->getCustomer()->getData('group_id') ?? 1 : 999;
            $collection
    ...
    public function load()
    {
        ...
        if (!$this->container->get(Request::class)->isAdmin()) {
            $visibleGroups = _mysql()->getTable("variant_group")
                ->addFieldToSelect("variant_group_id")
                ->where("visibility", "=", 1)
                ->fetchAllAssoc();
            $groups = [];
