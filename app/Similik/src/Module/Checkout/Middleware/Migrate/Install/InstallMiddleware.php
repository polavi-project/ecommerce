...
namespace Similik\Module\Checkout\Middleware\Migrate\Install;
...
class InstallMiddleware extends MiddlewareAbstract
{
    ...
    public function __invoke(Request $request, Response $response)
    {
        ...
        $cartTable = $this->processor->executeQuery("SELECT TABLE_NAME FROM information_schema.tables WHERE table_schema = :dbName AND TABLE_NAME = \"cart\" LIMIT 0,1", ['dbName'=> DB_DATABASE])->fetch(\PDO::FETCH_ASSOC);
        if($cartTable !== false) {
        ...
        try {
            //Create cart table
            $this->processor->executeQuery("CREATE TABLE `cart` (
            ...
            $this->processor->executeQuery("CREATE TABLE `cart_item` (
            ...
            $this->processor->executeQuery("CREATE TABLE `cart_address` (
            ...
            //Create order table
            $this->processor->executeQuery("CREATE TABLE `order` (
            ...
            $this->processor->executeQuery("CREATE TABLE `order_activity` (
            ...
            $this->processor->executeQuery("CREATE TABLE `payment_transaction` (
            ...
            $this->processor->executeQuery("CREATE TABLE `shipment` (
